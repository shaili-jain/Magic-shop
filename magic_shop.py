...
def recommend(n,t=top,based=option):
	...
	if based=='Director':
		dramas['comb'] =dramas['director']
		dramas['comb']=dramas['comb'].str.lower()
		vector1=dramas['director'].loc[idx]
	if based=='Screen Writer':
		dramas['comb'] =dramas['screenwriter']
		dramas['comb']=dramas['comb'].str.lower()
		vector1=dramas['screenwriter'].loc[idx]
	if based=='Director & Screen Writer':
		dramas['comb'] =dramas['screenwriter']+dramas['director']
		dramas['comb']=dramas['comb'].str.lower()
		vector1=dramas['screenwriter'].loc[idx]+dramas['director'].loc[idx]
	...
	for i in distances[1:t+1]:
		...
		recommended_screenwriter.append(df.iloc[i[0]].screenwriter)
		recommended_cast.append(df.iloc[i[0]].cast_names)
		recommended_synopsis.append(df.iloc[i[0]].synopsis)
	return recommended_names,recommended_posters,recommended_rating,recommended_aired,recommended_episodes,recommended_network,recommended_director,recommended_screenwriter,recommended_cast,recommended_synopsis,recommended_genre


if st.button('Recommend'):
	recommended_names,recommended_posters,recommended_rating,recommended_aired,recommended_episodes,recommended_network,recommended_director,recommended_screenwriter,recommended_cast,recommended_synopsis,recommended_genre = recommend(selected_dramas,top)
	for i in range(0,top):
		with st.container():
			if category_name=='Drama':
				...
				st.markdown('###### Screen Writer:')
				st.text(recommended_screenwriter[i])
				st.markdown('###### Cast:')
